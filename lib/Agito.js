...
function Agito() {
  ...
  this.start = function() {
    ...
    //   - first by port
    //   - then by hostname
    _triggers = _(_triggers)
      .groupBy(function(trigger) {
        return trigger.pattern && trigger.pattern.port;
      })
      .forEach(function(triggersOnPort, port, triggers) {
        triggers[port] = _(triggersOnPort)
          .groupBy(function(triggerOnPort) {
            return triggerOnPort.pattern && triggerOnPort.pattern.hostname;
          })
          .value()
        ;
      })
