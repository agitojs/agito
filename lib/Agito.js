...
function Agito() {
   ...
   * consumed by the `Agito#run()` method.
   *
   * @private
   * @type {Function[]}
   */
   ...
   * @returns {null}
   *
   * @throws {Error} if no middlewares are registered
  ...
  this.run = function() {
    ...
    async.eachSeries(_middlewares,
      ...
      function onAllDone(err) {
        ...
        // Create and start a listener on every ports
        ports = Object.keys(_triggers);
        listeners = ports.map(function(port) {
          var listener = new Listener(parseInt(port, 10));
          listener.start();
          return listener;
        });
      }
